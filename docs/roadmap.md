...
- **テンプレートは `SourceRoot` 配下に置く。テンプレート専用のルートを設けない。**
  `IContentResolver` に一本化する以上、テンプレートも `ContentPath` で名付けられなければならず、
  `Local` はサイト ルート配下しか表せない（9.1）。ルートを増やすのは 9.3 で却下した
  「複数ルート」そのものなので、ここでも採らない。ルート外のテーマを使いたい場合は
